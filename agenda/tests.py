...
class FormTests(TestCase):
    ...
    def reservable_clean_test(self, unit):
        ...
        print '--TEST EQUIPMENT PATRIMONY'
        equipment = Equipamento.objects.create(nome='b', descricao='d', unidade=unit, localizacao='a', patrimonio='123, 434, 12')
        try:
            equipment.clean()
        except:
            self.fail('Equipamento.clean() raised an exception unexpectedly!')
        equipment.delete()
        equipment = Equipamento.objects.create(nome='b', descricao='d', unidade=unit, localizacao='a', patrimonio='AWOIEFAWE?$(%)#212')
        self.assertRaises(ValidationError, lambda: equipment.clean())
        equipment.delete()
        print '--EQUIPMENT PATRIMONY TEST PASSED'
